...
class AppController extends Controller {
	...
	public function countRowsP($model, $field = null, $id = null){
		return ClassRegistry::init($model)->find('count', array('conditions'=>array($model.'.'.$field => $id)));
	}
	
	public function countRowsN($model, $field = null, $id = null){
		return ClassRegistry::init($model)->find('count', array('conditions'=>array('NOT'=>array($model.'.'.$field => $id))));
	}
	...
	public function createList($model, $parent_field, $secondary_field, $id_array = null){
		return ClassRegistry::init($model)->find('list', array('conditions'=>array($model.'.'.$parent_field => $id_array), 'fields'=> array($model.'.'.$parent_field, $model.'.'.$secondary_field)));
	}
	
	public function createListP($model, $parent_field, $secondary_field, $parent_field_1, $id_array = null){
		return ClassRegistry::init($model)->find('list', array('conditions'=>array($model.'.'.$parent_field_1 => $id_array), 'fields'=> array($model.'.'.$parent_field, $model.'.'.$secondary_field)));
	}
	
	public function getMinMax($model, $parent_field, $id){
		return ClassRegistry::init($model)->find('first', array('conditions'=>array($model.'.'.$parent_field => $id), 'fields' => array('MAX('.$model.'.price) as max_price', 'MIN('.$model.'.price) as min_price'), 'group' => $model.'.'.$parent_field));
	}
}
